...
#
# To run  code - 
#     python get_bus_info.py xxxx-xxx-xxxxx <BUS_LINE> <BUS_LINE>.csv
#
# BUS_LINE.csv file format - 
...
if not len(sys.argv) == 3:
    print("Invalid number of arguments. Run as: python show_bus_locations_skk45\
6.py <MTA_KEY> <BUS_LINE>")
    sys.exit()

# Store the MTA URL in variable v_mtaurl    

v_mtaurl = "http://bustime.mta.info/api/siri/vehicle-monitoring.json?key=" + sy\
s.argv[1] + "&VehicleMonitoringDetailLevel=calls&LineRef=" + sys.argv[2]
...
v_buses = v_text['Siri']['ServiceDelivery']['VehicleMonitoringDelivery'][0]['Ve\
hicleActivity']

# Get the number of buses in variable v_busno                                   
v_busno = np.size(v_text['Siri']['ServiceDelivery']['VehicleMonitoringDelivery'\
][0]['VehicleActivity'])
